...
const statusConfig: { [key in 'verified' | 'pending' | 'not_verified']: { bg: string; text: string; muted: string; } } = {
  verified: {
...
export function RentalStockView({ sections, unplacedTerminals, searchQuery, filter }: RentalStockViewProps) {
  ...
  if (!hasSections && filteredUnplaced.length === 0 && isFiltered) {
    // If filtering and no results anywhere, the parent component will show the "not found" message.
    return null;
  }
  ...
  return (
    <div className="space-y-6">
        <Card>
            <CardHeader><CardTitle>Стеллаж 12131</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                 {hasSections ? renderedSections : (
                    !isFiltered && <p className="text-muted-foreground col-span-full">Нет терминалов на стеллаже.</p>
                 )}
            </CardContent>
        </Card>
